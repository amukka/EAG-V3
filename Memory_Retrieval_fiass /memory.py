...
Vector retrieval via FAISS on top of keyword fallback. Reads go through
FAISS first (cosine similarity over the `embedding` field). When the
vector path returns nothing, the read falls back to keyword overlap.
Writes embed the descriptor at insert time for items of kind `fact`,
`preference`, and `tool_outcome`. Scratchpad items skip embedding.

Three design choices:
  1. Vector retrieval only (no hybrid sparse partner). Hybrid arrives later.
  2. Sliding-window chunking is heuristic. Semantic chunking arrives later.
  3. FAISS index reloaded from disk on every call for cross-process
     consistency with the MCP subprocess.
"""
...
def remember(
    ...
    *,
    ...
    # If the classifier returns an empty `value` dict, fall back to
    # {"raw": raw_text} so the originating text is always retrievable.
