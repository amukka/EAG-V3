"""agent.py — RAG Agent Orchestrator.

The loop layout connects the four layers:
    memory.read -> perception.observe -> decision.next_step ->
    action.execute -> memory.record_outcome

Run from this folder:
    python agent.py "What is the current time in Tokyo and Bangalore?"
"""
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            for it in range(1, MAX_ITERATIONS + 1):
                ...
                # Attach artifact bytes if Perception requested it.
