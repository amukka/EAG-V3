...
Session 7 architecture: MemoryItem carries an optional `embedding` field.
Items of kind `fact`, `preference`, and `tool_outcome` carry a vector embedding
written by Memory at insert time. The embedding underlies FAISS vector search.
Items of kind `scratchpad` are run-scoped and skip embedding.
"""
