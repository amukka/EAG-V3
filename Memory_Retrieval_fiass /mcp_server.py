"""
MCP server for Memory_Retrieval_fiass Session 7.
...
# Pull chunk settings from .env with fallback
try:
...
@mcp.tool()
def index_document(path: str, chunk_size: int | None = None, overlap: int | None = None) -> dict:
    ...
    csize = chunk_size if chunk_size is not None else CHUNK_SIZE
    coverlap = overlap if overlap is not None else CHUNK_OVERLAP
    
    text, source = _read_for_index(path)
    if not text.strip():
        return {"path": path, "source": source, "chunks_indexed": 0, "warning": "empty content"}
    chunks = _chunk_text(text, size=csize, overlap=coverlap)
