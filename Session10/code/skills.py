...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer":
        # Session 10: identical plug-in shape to the Browser skill. The
        # Computer-Use skill owns its cua-driver cascade (extract →
        # deterministic → a11y → vision) and never touches the LLM
        # text/tool channel, so we bypass render_prompt + gateway-chat and
        # hand off to ComputerSkill.run(NodeSpec).
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
            skill="computer",
            inputs=node_dict.get("inputs") or [],
