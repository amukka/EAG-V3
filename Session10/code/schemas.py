...
ErrorCode = Literal[
    ...
    # Session 10 (Computer-Use): the desktop analogue of gateway_blocked.
    # Raised when cua-driver returns an empty AX tree that no later layer can
    # recover — missing TCC grant, window not activated, Qt without
    # QT_ACCESSIBILITY, or a daemon that never started.
...
class ComputerOutput(BaseModel):
    """Session 10: typed payload the Computer-Use skill writes into
    AgentResult.output. The desktop twin of BrowserOutput.

    `path` is the cascade layer the skill actually used:
      extract       — read text straight off the AX tree / clipboard, no action
      deterministic — fixed hotkey / keystroke sequence, no LLM in the loop
      a11y          — AX-tree markdown + cheap text LLM judgment (scan-act-verify)
      vision        — screenshot + set-of-marks + vision LLM, click by (x, y)

    `app` / `bundle_id` name the target. `actions` is the per-turn record the
    replay viewer walks. `trajectory_dir` points at the cua-driver recording
    (the assignment's evidence). `verified` is the post-condition result — a
    click that returned success is not the same as a goal that was met.
    """
