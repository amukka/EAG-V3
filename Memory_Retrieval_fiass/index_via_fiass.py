...
from mcp_server import index_document, search_knowledge


def _candidate_export_dirs(root: Path) -> list[Path]:
    return [
        root / "sandbox" / "papers" / "url_text_exports",
        root / "S7code" / "sandbox" / "papers" / "url_text_exports",
        root / "url_text_exports",
    ]
...
def main() -> None:
    ...
    files: list[Path] = []
    seen: set[Path] = set()
    for src_dir in _candidate_export_dirs(root):
    ...
    if not files:
        print("No txt files found in url_text_exports")
        return

    print(f"\nIndexing {len(files)} files through FAISS via mcp_server.index_document...")
    total_chunks = 0
    for f in files:
        rel_path = str(f.relative_to(root / "sandbox"))
        res = index_document(rel_path)
    ...
    hits = search_knowledge(query, k=5)
    print(f"Hits returned: {len(hits)}")
    for i, h in enumerate(hits, 1):
        source = h.get("source", "")
        preview = h.get("chunk_preview", "").replace("\n", " ")[:180]
