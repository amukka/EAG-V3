...
# Explicit shared executor. asyncio.to_thread / run_in_executor(None, ...)
# lazily initialises the loop's DEFAULT executor, and that init is not
# thread-safe: when several skills fire concurrently (the parallel
# researcher fan-out) they race, and the loser raises
# "pop expected at most 1 argument, got 2". A pre-built shared executor
# sidesteps the lazy-init race entirely.
