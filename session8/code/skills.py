...
def _coerce_obj(parsed):
    """Normalise a parsed JSON value into the single top-level dict the
    orchestrator expects. Models (notably Gemini) sometimes return a bare
    JSON array `[{...}]` instead of an object `{...}`. Calling `.pop()` on
    that list downstream raised `TypeError: pop expected at most 1
    argument, got 2` and failed the node. We instead unwrap a single-item
    list, or wrap a multi-item list as the skill's `nodes`/`successors`."""
    if isinstance(parsed, dict):
        return parsed
    if isinstance(parsed, list):
        # A single object wrapped in a list → unwrap it.
        if len(parsed) == 1 and isinstance(parsed[0], dict):
            return parsed[0]
        # A list of node-specs → expose under both keys the lifter reads.
        if all(isinstance(x, dict) for x in parsed):
            return {"nodes": parsed, "successors": parsed}
    return {}


def parse_skill_json(text: str) -> dict:
    """Skills return a single top-level JSON object. Strip markdown fences
    if the model added them despite being told not to. Always returns a
    dict — see `_coerce_obj` for why a list reply must be normalised."""
    t = (text or "").strip()
    if t.startswith("```"):
        t = t.strip("`")
        t = t.split("\n", 1)[1] if "\n" in t else t
        if t.endswith("```"):
            t = t[:-3]
    try:
        return _coerce_obj(json.loads(t))
    except json.JSONDecodeError:
        # Try an object slice first, then an array slice.
        start, end = t.find("{"), t.rfind("}")
        if start >= 0 and end > start:
            try:
                return _coerce_obj(json.loads(t[start:end + 1]))
            except json.JSONDecodeError:
                pass
        start, end = t.find("["), t.rfind("]")
        if start >= 0 and end > start:
