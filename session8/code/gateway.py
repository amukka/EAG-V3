...
def ensure_gateway() -> None:
    ...
    # Wait for port to open (simple socket check is faster and more reliable)
    for attempt in range(120):
        time.sleep(0.5)
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            result = sock.connect_ex(('localhost', 8108))
            sock.close()
            if result == 0:
                # Port is open, give it another second to fully initialize
                time.sleep(1)
                print(f"[gateway] up on {GATEWAY_URL}")
                return
        except Exception:
            pass
    raise RuntimeError(f"Gateway V8 failed to start within 60s. Check {GATEWAY_V8_DIR}")
