...
def _ddg_search(query: str, max_results: int) -> list[dict]:
    """Search DuckDuckGo with SSL fallback."""
    try:
        return list(DDGS().text(query, max_results=max_results))
    except Exception as e:
        if ALLOW_INSECURE_FETCH and "CERTIFICATE_VERIFY_FAILED" in str(e):
            # Fallback: disable SSL verification temporarily
            import urllib3
...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    # When insecure fetch is allowed (ALLOW_INSECURE_FETCH), tell the headless
    # browser to ignore HTTPS/cert errors so sites with broken or self-signed
    # certs (e.g. macrotrends behind some networks) still fetch instead of
    # failing the whole researcher node. Built defensively: if this crawl4ai
    # build doesn't expose BrowserConfig, fall back to the plain crawler.
    crawler_kwargs = {"verbose": False}
    if ALLOW_INSECURE_FETCH:
        try:
            from crawl4ai import BrowserConfig
            crawler_kwargs = {"config": BrowserConfig(verbose=False,
