...
async def chat(req: ChatRequest):
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            # V8/V9: bounded same-provider retry on transient 5xx / timeout
            # before we fall through to the failover loop. Exponential backoff
            # capped at 2s. A single retry is not enough when a free-tier
            # provider (Gemini) returns 503 "model overloaded" in short bursts —
            # and with only one provider wired there is nothing to fail over to,
            # so the burst would otherwise abort the whole call. We retry up to
            # SAME_PROVIDER_RETRIES times. `retries` is surfaced in the response
            # so the orchestrator's replay can show how many were needed.
            ...
            while True:
                try:
                    ...
                    if not retryable or attempt >= SAME_PROVIDER_RETRIES:
                        raise
                    await _a.sleep(min(2.0, 0.5 * (2 ** attempt)))
                    attempt += 1
                    retries += 1
            latency = int((time.time() - t0) * 1000)
